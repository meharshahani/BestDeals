...
public class SoundSystemList extends HttpServlet {
	...
	/* Trending Page Displays all the Sound Systems and their Information in Game Speed */
	...
	/* Checks the Sound Systems type whether it is microsft or apple or samsung */
		...
		All the SoundSystem and its information are dispalyed in the Content Section
		...
		for (Map.Entry<String, SoundSystem> entry : hm.entrySet()) {
			SoundSystem sa = entry.getValue();
			if (i % 3 == 1)
				pw.print("<tr>");
			pw.print("<td><div id='shop_item'>");
			pw.print("<h3>" + sa.getName() + "</h3>");
			pw.print("<strong>" + sa.getPrice() + "$</strong><ul>");
			pw.print("<li id='item'><img src='images/soundSystem/"
					+ sa.getImage() + "' alt='' /></li>");
			...
			pw.print("<li><form method='post' action='WriteReview'>"+"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
					"<input type='hidden' name='type' value='sss'>"+
					"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
					"<input type='hidden' name='price' value='"+sa.getPrice()+"'>"+
					"<input type='hidden' name='access' value=''>"+
